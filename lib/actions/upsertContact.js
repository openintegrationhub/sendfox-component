...
const { upsertObject, getAccessToken } = require('./../utils/helpers');
...
async function processAction(msg, cfg) {
  try {
    ...
    */
    const transformedObject = transform(msg, cfg, personFromOih);

    // Execute the upsert operation
    const { success, responseId } = await upsertObject(transformedObject.data, token, recordUid);
