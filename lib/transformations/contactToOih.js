  ...
  if (msg.data.contact_fields) {
    for (let i = 0; i < msg.data.contact_fields.length; i += 1) {
      ...
      if (entry.name === 'phone_number') {
        ...
        if (entry.value) {
          entry.value = entry.value.replace(/[\s0]+/gui, ' ');
          ...
          if (parts.length < 2) {
            ...
            let subParts = parts[0].trim().split(' ');

            if (subParts.length > 0) {
              street = subParts.shift();
              if (subParts.length > 0) {
            ...
            if (parts.length > 1) {
              subParts = parts[1].trim().split(' ');

              if (subParts.length > 0) {
                zipcode = subParts.shift();
                if (subParts.length > 0) {
                  city = subParts.join(' ');
                }
              }
            ...
            if (parts.length > 2) {
              country = parts[2].trim(); // eslint-disable-line prefer-destructuring
            }
          }
        }
